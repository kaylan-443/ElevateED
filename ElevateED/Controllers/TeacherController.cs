...
namespace ElevateED.Controllers
{
    ...
    public class TeacherController : Controller
    {
        ...
        // Add this action to TeacherController
        [Authorize(Roles = "Teacher")]
        public ActionResult ActiveSessions()
        {
            using (var db = new ElevateEDContext())
            {
                ...
                var activeSessions = db.AttendanceSessions
                    .Include("Class")
                    .Include("Class.Grade")
                ...
                // Calculate present count for each session
                var sessionStats = new List<ActiveSessionViewModel>();
                foreach (var session in activeSessions)
                {
                    var totalStudents = db.AttendanceRecords.Count(r => r.AttendanceSessionId == session.AttendanceSessionId);
                    var presentCount = db.AttendanceRecords.Count(r => r.AttendanceSessionId == session.AttendanceSessionId && r.IsPresent);

                    sessionStats.Add(new ActiveSessionViewModel
                    {
                        SessionId = session.AttendanceSessionId,
                        ClassName = session.Class?.FullName ?? "Unknown",
                        OTPCode = session.OTPCode,
                        SessionDate = session.SessionDate,
                        OTPExpiry = session.OTPExpiry,
                        TotalStudents = totalStudents,
                        PresentCount = presentCount,
                        IsExpired = session.OTPExpiry < DateTime.Now
                    });
                }
