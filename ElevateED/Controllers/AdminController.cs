...
namespace ElevateED.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Dashboard()
        {
            ...
            // Students - use Class.Grade navigation property
            ...
            // Teachers
            viewModel.TotalTeachers = _context.Teachers.Count(t => t.IsActive);

            // Classes
            viewModel.TotalClasses = _context.Classes.Count();

            // Application Stats
            var apps = _context.Applicants.ToList();
            viewModel.ApplicationStats = new ApplicationStats
            {
                Total = apps.Count,
            ...
            // Approval Rate
            viewModel.ApprovalRate = viewModel.ApplicationStats.Total > 0
                ? Math.Round((double)viewModel.ApplicationStats.Approved / viewModel.ApplicationStats.Total * 100, 1)
                : 0;

            // Application Breakdown
            viewModel.ApplicationBreakdown = new List<ApplicationBreakdownItem>
    {
        new ApplicationBreakdownItem { Status = "Pending", Count = viewModel.ApplicationStats.Pending, Color = "#f59e0b" },
        new ApplicationBreakdownItem { Status = "Under Review", Count = viewModel.ApplicationStats.UnderReview, Color = "#3b82f6" },
        new ApplicationBreakdownItem { Status = "Approved", Count = viewModel.ApplicationStats.Approved, Color = "#10b981" },
        new ApplicationBreakdownItem { Status = "Rejected", Count = viewModel.ApplicationStats.Rejected, Color = "#ef4444" }
    };

            // Recent Applications
            viewModel.RecentApplications = apps
                ...
                {
                    FullName = a.FullName,
                    GradeApplyingFor = a.GradeApplyingForName ?? "Unknown",
                    Status = a.Status.ToString(),
                    DaysAgo = (int)(DateTime.Now - a.ApplicationDate).TotalDays
                })
                .ToList();

            // Grade Distribution - use navigation property NOT computed property
            viewModel.GradeDistribution = students
                .GroupBy(s => s.Class != null && s.Class.Grade != null ? s.Class.Grade.Name : "Unassigned")
                .ToDictionary(g => g.Key, g => g.Count());

            // Pending Tasks
            viewModel.PendingIssuesCount = _context.Issues.Count(i => i.Status == IssueStatus.Pending);
            viewModel.UnassignedClassesCount = _context.Classes.Count(c => c.ClassTeacherId == null);
            viewModel.PendingTeacherAssignments = _context.Teachers
                .Where(t => t.IsActive)
                .ToList()
                .Count(t => !t.SubjectAssignments.Any(a => a.IsActive));

            // Growth (placeholder - you can implement real calculation later)
            viewModel.StudentGrowth = 12;
            viewModel.TeacherGrowth = 5;
        ...
        public ActionResult ExamTimetables()
        {
            if (!IsAdmin()) return RedirectToAction("Login", "Account");

            try
            {
                var timetables = _context.ExamTimetables
                    .OrderByDescending(t => t.CreatedAt)
                    .ToList();

                var viewModels = timetables.Select(t => new ExamTimetableViewModel
                {
                    Id = t.Id,
                    Name = t.Name,
                    ...
                    StatusBadgeClass = GetStatusBadgeClass(t.Status),
                    CreatedAt = t.CreatedAt,
            ...
            {
                TempData["ErrorMessage"] = "Error loading timetables: " + ex.Message;
                return View(new List<ExamTimetableViewModel>());
            }
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateExamTimetable(CreateExamTimetableViewModel model)
        {
            if (!IsAdmin()) return RedirectToAction("Login", "Account");
        ...
        private string GetStatusDisplay(ExamTimetableStatus status)
        {
            switch (status)
            {
                case ExamTimetableStatus.Draft: return "Draft";
                case ExamTimetableStatus.AwaitingTeacherInput: return "Awaiting Teacher Input";
                case ExamTimetableStatus.Generated: return "Generated";
                case ExamTimetableStatus.Distributed: return "Distributed";
                case ExamTimetableStatus.Archived: return "Archived";
                default: return "Unknown";
            }
        }

        private string GetStatusBadgeClass(ExamTimetableStatus status)
        {
            switch (status)
            {
                case ExamTimetableStatus.Draft: return "badge-secondary";
                case ExamTimetableStatus.AwaitingTeacherInput: return "badge-warning";
                case ExamTimetableStatus.Generated: return "badge-primary";
                case ExamTimetableStatus.Distributed: return "badge-success";
                case ExamTimetableStatus.Archived: return "badge-dark";
